...
```
Ya la cantidad de paises que se encuentran en la base de datos es numerosa, no se
verá bien en un gráfico, por lo tanto solo visualizaremos el top 20.
...
Relación entre Género y Nivel Académico
Esto está pendiente, no confio en el resultado, me parece que tenemos un falso positivo,
Error tipo 1, plis soluciones.
Hipótesis:
Existen diferencias significativas en el nivel académico alcanzado entre géneros.
```{r}
...
```
Relación entre País y Área de Conocimiento
Hipótesis:
Ciertos países tienen una mayor concentración de personas en determinadas áreas de conocimiento.
```{r}
# Tabla de contingencia
...
Relación entre País y Género (Top 10 Países)
Hipótesis:
Existen diferencias significativas en la proporción de género entre los top 10 países.
```{r}
# Tabla de contingencia
...
```
Relación entre Edad y Nivel Académico
Hipótesis:
El nivel académico tiende a aumentar con la edad.
```{r}
...
Relación entre País, Estado Civil, y Género
Hipótesis:
Existen diferencias significativas en el estado civil según el género y el país.
```{r}
# Visualización
...
Hipótesis:
La edad tiene un impacto significativo en el estado civil de las personas.
```{r}
